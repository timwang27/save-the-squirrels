...
public class ShootAudioManager : MonoBehaviour
{
    ...
    void Awake()
    {
        foreach (ShootSound sound in sounds)
        {
    ...
    {
        ShootSound s = Array.Find(sounds, sound => sound.name == name);
        if (s == null)
        {
            Debug.Log("No music");
            return;
    ...
    {
        ShootSound s = Array.Find(sounds, sound => sound.name == audio);
    ...
    {
        ShootSound s = Array.Find(sounds, sound => sound.name == audioClip);
        if (s != null && s.source.isPlaying)
        {
            s.source.Stop();
        }
