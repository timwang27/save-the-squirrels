...
public class MazeAudioManager : MonoBehaviour
{
    ...
    void Awake()
    {
        foreach (MazeSound sound in sounds)
        {
    ...
    {
        MazeSound s = Array.Find(sounds, sound => sound.name == name);
        if (s == null)
        {
            Debug.Log("No music");
            return;
    ...
    {
        MazeSound s = Array.Find(sounds, sound => sound.name == audio);
    ...
    {
        MazeSound s = Array.Find(sounds, sound => sound.name == audioClip);
        if (s != null && s.source.isPlaying)
        {
            s.source.Stop();
        }
